...
int get(int l, int r, int i, int lq, int rq, int k) {
    //propagate(l, r, i);
    //if (lq <= l and r <= rq)
        //return seg[i];
    if (l == r)
    ...
    int rt = seg[c2].sum;
    if (k <= rt)
        return get(mid + 1, r, c2, lq, rq, k);
    else
        return get(l, mid, c1, lq, rq, k - rt);
    //merge(L, R);
}
...
void solve() {
    ...
    for (int i = n - 1; i >= 0; i--) {
        int k = v[i] + 1;
        int ret = get(0, n - 1, k);//get kth 1 from right 
        update(ret, 0);
        res[i] = ret + 1;
    }
    for (auto i : res)
